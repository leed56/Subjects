...
select
  ...
  unit_count,
  description,
  (case when public.can_see_org_financials(organization_id) then quoted_amount else 0 end)::numeric(14, 2) as quoted_amount,
  (case when public.can_see_org_financials(organization_id) then deposit_amount else 0 end)::numeric(14, 2) as deposit_amount,
  pipe_meters,
  status,
  scheduled_date,
  installed_date,
