...
const PHARMACY_RETAIL_POSITIVE = /\b(?:BABY|INFANT|DIAPER|NAPPY|WIPES?|FEEDING|PACIFIER|TEETHER|VITAMIN|MULTIVITAMIN|COLLAGEN|BIOTIN|SUPPLEMENT|WHEY|PROTEIN|NUTRITION|ENSURE|APPETON|DIABETASOL|GLUCERNA|GLUCOMETER|BLOOD PRESSURE|THERMOMETER|LANCET|NEBULIZER|NEBULISER|WHEEL\s*CHAIR|WHEELCHAIR|SUPPORT|BRACE|MASK|TOOTHPASTE|TOOTHBRUSH|MOUTHWASH|ORAL|SHAMPOO|CONDITIONER|HAIR|SOAP|BODY WASH|SHOWER GEL|DEODORANT|SANITARY|INTIMATE|LOTION|SERUM|SUNSCREEN|MOISTURIZER|MOISTURISER|FACE WASH|CLEANSER|SKIN CREAM|FACE CREAM|HAND CREAM|BODY CREAM|BEAUTY CREAM|DAY CREAM|NIGHT CREAM|BB CREAM|CC CREAM|BISCUIT|CRACKER|CHOCOLATE|CANDY|GUM|MINT|CHIPS|SNACK|WATER|SOFT DRINK|FRUIT DRINK|JUICE|ENERGY DRINK|SPORTS DRINK|MILK DRINK|TISSUE|DISINFECTANT|SANITIZER|SANITISER|MOSQUITO|REPELLENT)\b/i;

export function isPharmacyRetailCandidate(name) {
  const text = String(name ?? "");
  if (WELLNESS.test(text)) return true;
