...
function jobVars(row: AcJobRow, business: BusinessInfo) {
  const balance = row.quoted_amount - row.deposit_amount;
  return {
    ...
    quotedAmount: formatLkr(row.quoted_amount),
    depositAmount: formatLkr(row.deposit_amount),
...
export async function runServiceDueReminders(
  ...
  for (const org of (orgs ?? []) as OrgRow[]) {
    ...
    const targetDates = settings.serviceDueRemindDays.map((d) => addDays(today, d));
