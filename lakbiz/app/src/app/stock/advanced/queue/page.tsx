...
export default function TrackedReceivingQueuePage() {
  ...
  const rows = useMemo(() => {
    ...
    return coverage
      ...
      .sort((a, b) => b.pending - a.pending || a.product.name.localeCompare(b.product.name));
  }, [coverage, data]);

  const pending = rows.filter((row) => row.pending > 0);
  const complete = rows.filter((row) => row.pending <= 0);
    ...
    <AppShell>
      <ProMain>
        <PageHeader
          ...
          description={
            si
              ? "GRN, PO receive හෝ Stock In පසු batch / variant / IMEI identity තවම නොදමා ඇති stock මෙහි පෙන්වයි."
              : "See stock that has already been received through GRN, PO receiving or Stock In but still needs batch, variant or IMEI identity."
          }
          actions={
          ...
          <div className="space-y-5">
            <section className="grid gap-4 sm:grid-cols-3">
              <div className="rounded-2xl bg-slate-950 p-5 text-white shadow-[0_12px_32px_rgba(15,23,42,0.12)]">
                <p className="text-[10px] font-bold uppercase tracking-[0.15em] text-slate-400">{si ? "Action needed" : "Action needed"}</p>
                <p className="mt-2 text-3xl font-semibold">{pending.length}</p>
                <p className="mt-1 text-sm text-slate-300">{si ? "tracked products" : "tracked products"}</p>
              </div>
              ...
              <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-5">
                <p className="text-[10px] font-bold uppercase tracking-[0.15em] text-emerald-700">{si ? "Complete" : "Complete"}</p>
                <p className="mt-2 text-3xl font-semibold text-emerald-950">{complete.length}</p>
                ...
                <EmptyState
                  ...
                  description={
                    rows.length === 0
                      ...
                      : si ? "සියලුම on-hand tracked stock සඳහා exact identity සටහන් වී ඇත." : "Every on-hand tracked item has its required exact inventory identity."
                  }
            ...
            {complete.length > 0 && (
              <section className={card}>
                <div className="flex flex-wrap items-center justify-between gap-3">
                  ...
                  <StatusBadge tone="positive">{complete.length}</StatusBadge>
                </div>
