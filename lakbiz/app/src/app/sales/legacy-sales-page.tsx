                ...
                {fastRetailPos && (
                  <>
                    ...
                    <div className="mt-3 flex gap-2 overflow-x-auto pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
                      <button type="button" onClick={() => { setCategoryFilter("all"); setResultLimit(24); }} className={`shrink-0 rounded-xl px-3 py-2 text-xs font-bold ${categoryFilter === "all" ? "bg-teal-600 text-white" : "border border-slate-200 bg-white text-slate-600"}`}>{si ? "සියල්ල" : "All"} <span className="opacity-70">{inStock.length}</span></button>
                      ...
                      >
                        <div className="flex items-start justify-between gap-3">
                          <div className="min-w-0">
                            ...
                            {org.sector === "pharmacy" && (
                              <p className="mt-1 line-clamp-1 text-[11px] font-medium text-slate-500">{[p.customFields.genericName, p.customFields.strength, p.customFields.brand, p.customFields.packSize].filter(Boolean).map(String).join(" · ") || (si ? "Batch පාලිත තොග" : "Batch-controlled stock")}</p>
                            )}
                          </div>
