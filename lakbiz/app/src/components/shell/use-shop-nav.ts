...
/** Filters the shared nav model by route access, plan/addon features and the
 * provisioned business sector. Desktop and MobileNav consume this same result. */
export function useShopNav(): {
  sections: { labelKey: string | null; items: NavItem[] }[];
  managementItems: NavItem[];
} {
  const { can, canAccessShopRoute, canAccessSettingsPath, org } = useSubscription();
  ...
  }, [can, canAccessShopRoute, canAccessSettingsPath, org.sector]);
}
