...
type CheckoutPayment = Exclude<PaymentMethod, "mixed">;
...
export function AtomicRetailSalesPage() {
  ...
  useEffect(() => {
    try {
      ...
      if (!raw) return;
      const saved = JSON.parse(raw) as { saleId?: string; billNo?: string };
      sessionStorage.removeItem(SUCCESS_STORAGE_KEY);
      if (saved.saleId) setLastBillId(saved.saleId);
      setMessage(
        si
          ? `විකිණීම සම්පූර්ණයි${saved.billNo ? ` · ${saved.billNo}` : ""}. තොගය සහ ගෙවීම එකම transaction එකක සුරකින ලදී.`
          : `Sale completed${saved.billNo ? ` · ${saved.billNo}` : ""}. Payment and inventory were committed in one transaction.`,
      );
    } catch {
      sessionStorage.removeItem(SUCCESS_STORAGE_KEY);
    }
  ...
  const handleSale = async () => {
    ...
    if (payment === "cash" && cashReceived !== "" && Number(cashReceived) < netTotal) {
      setMessage(si ? "ලැබුණු මුදල විකිණීමේ මුළු වටිනාකමට වඩා අඩුයි." : "Cash received is below the sale total.");
      return;
    }
    ...
    if (payment === "credit" && selectedCustomer?.creditLimit != null) {
      const nextBalance = selectedCustomer.creditBalance + netTotal;
    ...
    if (payment === "cheque" && (!chequeNo.trim() || !chequeBank.trim() || !chequeDate)) {
    ...
    const validation = validateSaleTenders([tender], {
      saleTotal: netTotal,
    ...
    const tenderSummary = summarizeSaleTenders(netTotal, [tender]);
    if (!tenderSummary.settled) {
      setMessage("Payment allocation does not settle the invoice.");
      return;
    }
    ...
    // The v3 finalizer increments organizations.sync_generation after commit,
    // so an older browser snapshot cannot legitimately overwrite this sale.
    // Pull the authoritative masked views immediately, persist them locally,
    // then perform a full document reload so AppStore remounts from the fresh
    // snapshot instead of showing stale stock/recent-sales state.
    const fresh = await pullBusinessData(org.id, data.business).catch(() => null);
    if (fresh) saveAppData(fresh, org.id);
    sessionStorage.setItem(
      SUCCESS_STORAGE_KEY,
      JSON.stringify({ saleId: result.saleId, billNo: result.billNo }),
    );
    ...
    <AppShell>
      <ProMain>
        <ProPageHeader
          ...
          actions={
            <>
              <ProBadge tone="emerald">{si ? "Atomic checkout" : "Atomic checkout"}</ProBadge>
            ...
            <aside className="space-y-4 xl:sticky xl:top-5 xl:self-start">
              <ProCard title={t("sales.cart_title")} eyebrow={t("sales.checkout_eyebrow")}>
                ...
                <div className="mt-5 space-y-4">
                    ...
                    <div className="mt-2 grid grid-cols-2 gap-2">
                      {(PAYMENT_OPTIONS as CheckoutPayment[]).map((method) => (
                        <button
                          key={method}
                          type="button"
                          onClick={() => { setPayment(method); setPendingSaleId(null); }}
                  ...
                  >
                    {saving ? t("common.saving") : identityBlocked ? (si ? "Checkout අවශ්‍යතා සම්පූර්ණ කරන්න" : "Complete checkout requirements") : t("sales.complete")}
                  </button>
                </div>
              </ProCard>
            </aside>
                  ...
                  <tbody>
                    {data.sales.slice(0, 10).map((sale) => (
                      <tr key={sale.id} className="border-b last:border-0">
                        ...
                        <td className="px-4 py-3"><ProBadge tone="slate">{paymentLabel(t, sale.paymentMethod)}</ProBadge></td>
